...
export default class ModalContent extends React.Component {
    ...
    componentDidUpdate(oldProps, oldState) {
        ...
        if (list[this.state.index] !== oldProps.id[oldState.index]) {
            this.handleUpdateLang(this.lang)
            this.props.onActiveUpdate(list[this.state.index], oldProps.id[oldState.index])
        }
    }
            ...
            <div>
                ...
                <div className="i18n-lang-context">
                    <button
                        ...
                        onClick={() => { transaction.register('en-US'); this.forceUpdate() }}
                    >
                        en-US
                    </button>
                    <button
                        ...
                        onClick={() => { transaction.register('zh-CN'); this.forceUpdate() }}
                    >
                        zh-CN
                    </button>
                </div>
                <div className={bodyPrefix + 'info'}>
                    ...
                    <div className={bodyPrefix + 'key'}>
                        <span>Key: </span><span>{idList[index]}</span>
                    </div>
                    {
                        this.srcList[index] &&
                        <div className={bodyPrefix + 'key'}>
                            <span>Src: </span><span>{this.srcList[index]}</span>
                        </div>
                    }
                    {
                ...
                <Footer>
                    <span className="">
                        <button
                            className="i18n-modal-btn i18n-icon-left" title={'previous'}
                            onClick={() => {
                                this.setState({
                        ...
                        >{'<'}</button>
                        <button
                            className="i18n-modal-btn i18n-icon-right" title={'next'}
                            onClick={() => {
                                this.setState({
                                    index: this.state.index + 1,
                                })
                    ...
                    <span className="i18n-right">
                        ...
                        <button
                            disabled={this.state.fetching || !value[index] || value[index] === rawList[index]}
