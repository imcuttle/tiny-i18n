      ...
      if (typeof name === 'string' && props) {
        children = children.map((child, index, { length }) => {
          if (typeof child === 'string') {
            return spiderEntity(
              child,
              json => handleValue(json, 'children', !(
                length === 1 && child === child
              ))
            )
          }
          return child
                ...
                <ModalContent
                  ...
                  onSave={async data => {
                    let passed = await transaction.update({
                      id: data.data.id, value: data.value, source: data.data.src
                    })
                    if (passed) {
                      const i = ctx.content.state.index
                      if (ctx.content.lang === core.default.getLang().lang.replace(/_/g, '-')) {
                        core.default.setI18nVals({ [data.data.id]: data.value }, data.data.src)
                        updateDOM(target, data.data.id, vendor.raw[i], _oldi(data.data.id, ...data.data.args))
...
function _i(key, ...args) {
  ...
  args = args.map(arg =>
    arg.replace
      ? spiderEntity(arg, json => (
        (
          list = list.concat(json)
        ),
          `${NULL_CHAR}${json.raw}${NULL_CHAR}`
      ))
      : arg
  )
...
if (!global.location) {
  ...
  if ((
        query && query.hasOwnProperty('i18n-edit-live')
      ) || !!localStorage.getItem('i18n-edit-live')) {
